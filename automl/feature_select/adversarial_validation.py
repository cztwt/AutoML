...
class AdversarialValidation:
    """对抗性验证(Adversarial Validation)是一种用于检测训练集和测试集分布差异的技术

    利用该方法进行特征筛选步骤：
        1. 构建二分类器对训练集和测试集进行区分，即将训练集和测试集样本分别标记为0和1
        2. 
    ...
    def fit(self, train, test, target, categorical_features=[], p=0.6):
        """
        ...
        n_fold = 5
        folds = KFold(n_splits=n_fold, shuffle=True, random_state=2024)
        ...
        while True:
            logger.info(f'used_features: {used_features}')
            ...
            for fold_n, (train_index, valid_index) in enumerate(folds.split(train_test[used_features])):
                start_time = time()
                logger.info('Training on fold {}'.format(fold_n + 1))
                ...
                logger.info('AUC: {}'.format(auc_score))
                AUCs.append(auc_score)
                logger.info('Fold {} finished in {}'.format(fold_n + 1, str(datetime.timedelta(seconds=time() - start_time))))
            ...
            if mean_auc > p:
                ...
                logger.info(f"remove feature {cur_removed_feature}")
                ...
                logger.info(f"removed_features: {self.removed_features}")
                break
